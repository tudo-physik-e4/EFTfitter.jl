...
function nounc(params)
    return 0
end

function xsec1_unc(params)
    return 0.1*xsec1(params)
end
...
diff_xsec = [Observable(diff_xsec_bin1, nounc), Observable(diff_xsec_bin2, nounc), Observable(diff_xsec_bin3, nounc)]
...
measurements = (
    Meas1 = Measurement(Observable(xsec1, xsec1_unc), 21.6, uncertainties = (stat=0.8, syst=1.8, another_unc=2.3),
            active=true), # `active = false`: exclude measurement from fit (default: active = true)

    Meas2 = Measurement(Observable(xsec2, nounc,  min=0), 1.9,
